...
def investigar_site(dominio):
    ...
    try:
        with sync_playwright() as p:
            ...
            # Limpa e filtra apenas os emails que parecem válidos para a empresa
        ...
        # AGORA ELE VAI MOSTRAR O ERRO EM VEZ DE O ESCONDER
...
if iniciar:
    if not ficheiro_txt or not password_app:
        ...
        # Filtra os válidos e mostra no ecrã
        df_all = pd.DataFrame(dados_finais)
        df_validos = df_all[df_all['Email_Status'].str.contains('✅|⚠️|❓', na=False)]
        
        # Mostra a tabela com os resultados na página
        with st.expander("Ver lista de Emails Encontrados e Validados", expanded=True):
            st.dataframe(df_all[['Domínio', 'Emails_Site', 'Email_Status']])
            
        st.write(f"📊 Encontrados **{len(df_validos)}** emails válidos para contacto.")

        # --- FASE 3: ENVIO DE EMAILS ---
        if df_validos.empty:
            st.error("❌ Nenhum email válido foi encontrado nesta lista de domínios. A campanha parou aqui.")
        else:
